  ...
  <>
        ...
        <div className="flex items-end gap-2 h-44">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="flex-1 flex justify-center">
              <Skeleton className="w-4 rounded-t-md" style={{ height: `${20 + Math.random() * 60}%` }} />
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="mt-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {[0, 1, 2].map((i) => (
          <div key={i} className="glass-panel rounded-[28px] p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between"><Skeleton className="h-3 w-20" /><Skeleton className="h-4 w-14" /></div>
            </div>
          </div>
        ))}
      </div>
